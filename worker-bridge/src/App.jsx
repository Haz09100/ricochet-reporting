...
const defaultOptions = { statuses: [], agents: [], vendors: [], lead_types: [], states: [], cities: [] };
...
function FilterPanel({ draft, setDraft, apply, options, open, setOpen }) {
  ...
  return <section className="filter-card">
    ...
    {open && <><div className="filter-grid expanded">
...
export default function App() {
  ...
  const [data, setData] = useState(() => demoForPage("overview"));
  ...
  const setPage = (next) => {
    setPageState(next); setSidebarOpen(false); setPagination({ page: 1, pageSize: 50 });
  ...
  useEffect(() => {
    const syncRoute = () => {
      ...
      if (navigation.some(([key]) => key === next)) { setPageState(next); setPagination({ page: 1, pageSize: 50 }); }
    };
  ...
  const load = useCallback(async ({ background = false } = {}) => {
    ...
    setError("");
    try {
      const next = preview ? demoForPage(page) : await loadReportPage(page, filters, pagination);
