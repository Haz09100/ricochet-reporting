...
import { loadAllFilteredCalls, loadAllFilteredLeads, loadCallAiReview, loadCsvCallDetails, loadLiveBonusReview, matchCsvRows, runAiAction, setCompanionBonusDecision, setLiveBonusDecision } from "../lib/reportApi.js";
...
function CompanionLeadDetailsPopup({ rows, selectedLeadId, setSelectedLeadId }) {
  const selectedIndex = rows.findIndex((row) => String(row.id) === String(selectedLeadId));
  ...
  const move = (offset) => {
    ...
    const next = (selectedIndex + offset + rows.length) % rows.length;
    setSelectedLeadId(rows[next].id);
  };
  const deliveryStatus = String(row.delivery_status || row.routing_status || "unknown").toLowerCase();
  const delivered = deliveryStatus === "delivered";
  const tags = Array.isArray(row.sent_tags) ? row.sent_tags : [];
  const trackingTags = Array.isArray(row.source_tracking_tags) ? row.source_tracking_tags : [];
  const generatedNote = cleanNoteText(row.generated_note);
  const propertySearch = row.sent_property_search && typeof row.sent_property_search === "object" ? JSON.stringify(row.sent_property_search,null,2) : "";
  return <div className="review-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedLeadId(null); }}><section className="review-modal companion-detail-modal" role="dialog" aria-modal="true" aria-label={`Companion delivery details for ${row.lead_name}`}>
    <header className="review-modal-header"><div><span className="eyebrow">Created companion lead</span><h2>{row.lead_name}</h2><div className="tag-row"><span className="tag">{row.creation_label}</span><span className={`delivery-state ${delivered ? "delivered" : "pending"}`}>{delivered ? "FUB received" : deliveryStatus.replaceAll("_"," ")}</span><span className={`bonus-state ${row.bonus_state}`}>{String(row.bonus_state || "unknown").replaceAll("_"," ")}</span></div></div><button className="review-close" onClick={() => setSelectedLeadId(null)} aria-label="Close companion details"><X size={22} /></button></header>
    <div className="review-modal-body companion-detail-body">
      <div className={`delivery-proof ${delivered ? "confirmed" : "unconfirmed"}`}><CheckCircle2 size={20} /><div><strong>{delivered ? "LeadFlow confirms FUB received this companion lead" : "FUB receipt is not confirmed"}</strong><span>{delivered ? `Created as FUB person ${row.destination_fub_person_id || "ID not recorded"}${row.fub_account_name ? ` in ${row.fub_account_name}` : ""}.` : row.delivery_error || `Current delivery state: ${deliveryStatus}.`}</span></div></div>
      <div className="review-summary-grid">
        {[ ["Created",formatNoteTime(row.created_at)], ["LeadFlow ID",row.source_lead_id], ["FUB person ID",row.destination_fub_person_id], ["FUB account",row.fub_account_name], ["Agreement",row.agreement_name], ["Stage sent",row.sent_stage], ["Source sent",row.sent_source], ["Delivery attempts",row.delivery_attempts], ["Lead type",row.lead_type], ["Originating ISA",row.originating_agent], ["Split group",row.split_group_id], ["Location",[row.city,row.state].filter(Boolean).join(", ")] ].map(([label,content]) => <div className="review-fact" key={label}><span>{label}</span><strong>{value({ content },"content")}</strong></div>)}
      </div>
      <section className="review-section"><div className="review-section-heading"><div><span className="eyebrow">Generated form note</span><h3>{row.lead_type} note attached to the FUB delivery</h3></div></div><div className="companion-note-body">{generatedNote || "No generated background/note was recorded in the LeadFlow delivery snapshot."}</div></section>
      <section className="review-section"><div className="review-section-heading"><div><span className="eyebrow">FUB payload</span><h3>Message, tags, and routing details</h3></div></div>
        <div className="companion-detail-grid"><div><span>Activity message</span><p>{row.sent_message || "No separate activity message recorded."}</p></div><div><span>Delivery state</span><p>{deliveryStatus.replaceAll("_"," ")}</p></div><div><span>Allocation ID</span><p>{row.allocation_id || "—"}</p></div><div><span>Routing state</span><p>{row.routing_status || "—"}</p></div></div>
        <span className="companion-field-label">Tags sent to FUB</span><div className="companion-tags">{tags.length ? tags.map((tag) => <span className="tag" key={tag}>{tag}</span>) : <span className="muted">No tags were recorded.</span>}</div>
        {row.delivery_error && <ErrorBox message={`Delivery error: ${row.delivery_error}`} />}
      </section>
      {(row.buyer_search_activity_status || propertySearch) && <section className="review-section"><div className="review-section-heading"><div><span className="eyebrow">Buyer search activity</span><h3>Property-search details sent with the buyer</h3></div></div><div className="companion-detail-grid"><div><span>Status</span><p>{String(row.buyer_search_activity_status || "unknown").replaceAll("_"," ")}</p></div><div><span>FUB response</span><p>{row.buyer_search_activity_response_code || "—"}</p></div></div>{row.buyer_search_activity_error && <ErrorBox message={row.buyer_search_activity_error} />}{propertySearch && <pre className="companion-json">{propertySearch}</pre>}</section>}
      {(row.source_tracking_status || trackingTags.length || row.source_tracking_error) && <section className="review-section"><div className="review-section-heading"><div><span className="eyebrow">Original FUB contact tracking</span><h3>Tags attached back to the originating contact</h3></div></div><div className="companion-detail-grid"><div><span>Status</span><p>{String(row.source_tracking_status || "unknown").replaceAll("_"," ")}</p></div><div><span>Person ID</span><p>{row.source_tracking_person_id || "—"}</p></div><div><span>Lookup method</span><p>{row.source_tracking_lookup_method || "—"}</p></div><div><span>Updated</span><p>{formatNoteTime(row.source_tracking_at)}</p></div></div><div className="companion-tags">{trackingTags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div>{row.source_tracking_error && <ErrorBox message={row.source_tracking_error} />}</section>}
    </div>
    <footer className="review-modal-footer"><div className="review-navigation"><button className="button secondary" onClick={() => move(-1)}><ChevronLeft size={16} />Previous</button><button className="button secondary" onClick={() => move(1)}>Next lead<ChevronRight size={16} /></button></div><span className="muted">Delivery details come directly from LeadFlow D1.</span></footer>
  </section></div>;
}
...
export function OverviewView({ data }) {
  ...
  const cards = [
    ...
    ["Live leads sent", totals.live_leads_sent, "Matches the audited Team ledger", "green"],
    ["Live emails sent", totals.live_emails_sent, "Same qualified live-lead total", "mint"],
...
export function TeamView({ data, setToast, onDataChanged }) {
  ...
  </article><LiveBonusReport data={data} setToast={setToast} onDataChanged={onDataChanged} /><CompanionBonusReport data={data?.companion_bonus} setToast={setToast} onDataChanged={onDataChanged} /></>;
}

function CompanionBonusReport({ data, setToast, onDataChanged }) {
  ...
  const visible = originFilter === "all" ? ledger : ledger.filter((row) => row.creation_origin === originFilter);
  const agentSort = useTableSort(agents, {
    agent: { value: "agent" }, payable: { value: "payable_companion_leads", type: "number" },
    buyers: { value: "buyers_from_seller", type: "number" }, sellers: { value: "sellers_from_buyer", type: "number" }, approved: { value: "manager_approved", type: "number" },
  }, "payable", "desc");
  const ledgerSort = useTableSort(visible, {
    ...
    origin: { value: "creation_label" }, delivery: { value: "delivery_status" }, agent: { value: "credited_agent_name" }, state: { value: "bonus_state" },
  }, "created", "desc");
  ...
  const cards = [
    ["Companion leads created",totals.created,"Buyer + seller companions"],
    ["Buyers from Seller Forms",totals.buyers_from_seller,"Generated buyer side"],
    ["Sellers from Buyer Forms",totals.sellers_from_buyer,"Generated seller side"],
    ["Payable companion bonuses",totals.payable,"Credited to the form ISA"],
    ["Needs review",totals.needs_review,"Originating ISA missing"],
  ...
  return <section className="live-ownership-section companion-bonus-section">
    ...
    <article className="panel report-panel bonus-review-panel"><div className="panel-heading"><div><span className="eyebrow">Created companion leads</span><h3>Review and pay generated buyer/seller opportunities</h3><p>Click a lead to open its complete record. Use the buttons to correct the credited agent or retract a companion bonus.</p></div><div className="segmented"><button className={originFilter === "all" ? "active" : ""} onClick={() => setOriginFilter("all")}>All</button><button className={originFilter === "seller_to_buyer" ? "active" : ""} onClick={() => setOriginFilter("seller_to_buyer")}>Created buyers</button><button className={originFilter === "buyer_to_seller" ? "active" : ""} onClick={() => setOriginFilter("buyer_to_seller")}>Created sellers</button></div></div>
      {!visible.length ? <Empty message="No companion leads matched this selection." /> : <div className="table-wrap"><table><thead><tr><SortHeader id="lead" label="Lead" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="created" label="Created date" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="type" label="Lead type" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="origin" label="Created from" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="delivery" label="FUB delivery" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="agent" label="Bonus agent" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><SortHeader id="state" label="Bonus state" sort={ledgerSort.sort} onSort={ledgerSort.requestSort} /><th>Actions</th></tr></thead><tbody>{ledgerSort.rows.map((row) => <tr key={row.id}><td><button className="lead-review-link" onClick={() => setSelectedLeadId(row.id)}><strong>{row.lead_name || `Lead ${row.source_lead_id || row.id}`}</strong><small>{row.source_system === "leadflow-d1" ? `LeadFlow ID ${row.source_lead_id} · Open FUB details` : `ID ${row.id} · Open full record`}</small></button></td><td>{value(row,"created_date_eastern")}</td><td>{value(row,"lead_type")}</td><td><span className="tag">{value(row,"creation_label")}</span></td><td><span className={`delivery-state ${row.delivery_status === "delivered" ? "delivered" : "pending"}`}>{String(row.delivery_status || "unknown").replaceAll("_"," ")}</span>{row.destination_fub_person_id && <small className="delivery-person-id">FUB #{row.destination_fub_person_id}</small>}</td><td>{value(row,"credited_agent_name")}</td><td><span className={`bonus-state ${row.bonus_state}`}>{String(row.bonus_state || "unknown").replaceAll("_"," ")}</span></td><td>{canManage ? <div className="bonus-actions">{row.bonus_state === "needs_review" && <button className="text-action" disabled={busyLead === row.id} onClick={() => requestDecision(row,"approved")}>Approve agent</button>}{row.bonus_state !== "retracted" && <button className="text-action danger" disabled={busyLead === row.id} onClick={() => requestDecision(row,"retracted")}>Retract</button>}{row.bonus_state === "retracted" && <button className="text-action" disabled={busyLead === row.id} onClick={() => requestDecision(row,"reset")}>Restore</button>}</div> : <span className="muted">Manager only</span>}</td></tr>)}</tbody></table></div>}
    </article>
    {selectedLeadId && (ledgerSort.rows.find((row) => String(row.id) === String(selectedLeadId))?.source_system === "leadflow-d1" ? <CompanionLeadDetailsPopup rows={ledgerSort.rows} selectedLeadId={selectedLeadId} setSelectedLeadId={setSelectedLeadId} /> : <LeadReviewPopup rows={ledgerSort.rows} selectedLeadId={selectedLeadId} setSelectedLeadId={setSelectedLeadId} />)}
...
function LiveBonusReport({ data, setToast, onDataChanged }) {
  ...
  const visible = stateFilter === "all" ? ledger : ledger.filter((row) => stateFilter === "waiting_or_missing"
    ? ["waiting_for_note","missing_formal_note","missing_isa","missing_live_disposition"].includes(row.bonus_state)
    : row.bonus_state === stateFilter);
  const agentSort = useTableSort(agents, {
  ...
  const evidenceMissing = Number(totals.waiting_for_note || 0) + Number(totals.missing_formal_note || 0) + Number(totals.missing_isa || 0) + Number(totals.missing_live_disposition || 0);
  const cards = [
    ...
    ["Incomplete note evidence", evidenceMissing, "Missing form, ISA, or disposition"],
    ["Retracted", totals.retracted, "Removed by manager"],
  ];
  useEffect(() => {
...
function LiveBonusReviewModal({ row, detail, loading, error, canManage, busy, onClose, onPrevious, onNext, onApprove, onReject, bonusMode = true }) {
  ...
  if (bonusMode) facts.push(["Formal-note owner",row?.note_owner],["Form ISA",row?.form_isa],["Evidence reason",row?.gate_reason]);
...
const leadExportFields = [
  ...
  { key: "creation_origin", label: "Lead Origin", defaultSelected: true, value: (row) => row.creation_label || "Original / not companion" },
  { key: "originating_agent", label: "Companion Originating Agent", value: (row) => row.creation_origin === "original" ? "" : row.originating_agent || "" },
...
export function CsvView({ setToast }) {
  const emptyFilters = { search: "", match: "", status: "", type: "", origin: "", vendor: "", agent: "" };
  ...
  const options = useMemo(() => {
    ...
    return { match: values("match_status"), status: values("lead_status"), type: values("lead_type"), origin: values("creation_label"), vendor: values("vendor"), agent: values("user_name") };
  }, [matches]);
  const visibleMatches = useMemo(() => {
    ...
    return matches.filter((row) => {
      ...
      if (filters.origin && row.creation_label !== filters.origin) return false;
      if (filters.vendor && row.vendor !== filters.vendor) return false;
      if (filters.agent && row.user_name !== filters.agent) return false;
      if (!search) return true;
      return [row.lead_id,row.input_first_name,row.input_last_name,row.matched_first_name,row.matched_last_name,row.input_phone,row.matched_phone,row.input_email,row.matched_email,row.lead_status,row.lead_type,row.creation_label,row.originating_agent,row.vendor,row.user_name].filter(Boolean).join(" ").toLowerCase().includes(search);
    });
  }, [matches, filters]);
  const sorted = useTableSort(visibleMatches, {
    ...
    status: { value: "lead_status" }, type: { value: "lead_type" }, origin: { value: "creation_label" }, calls: { value: "call_count", type: "number" }, notes: { value: "note_count", type: "number" }, agent: { value: "user_name" },
  });
