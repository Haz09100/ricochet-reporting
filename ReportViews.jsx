...
export function TeamView({ data }) {
  ...
  return <article className="panel report-panel"><div className="panel-heading"><div><span className="eyebrow">Agent performance</span><h3>One fast, pre-aggregated Supabase summary</h3></div><div className="segmented"><button className={mode === "calls" ? "active" : ""} onClick={() => setMode("calls")}>By calls</button><button className={mode === "notes" ? "active" : ""} onClick={() => setMode("notes")}>By notes</button></div></div>
    {!rows.length ? <Empty message="No agent activity matched these filters." /> : <div className="table-wrap"><table><thead><tr>{mode === "calls" ? <><th>Caller</th><th>Score</th><th>Calls</th><th>Unique leads</th><th>Handled</th><th>Avg duration</th><th>First call</th><th>Last call</th></> : <><th>Note author</th><th>Notes</th><th>Unique leads</th><th>First note</th><th>Last note</th></>}</tr></thead><tbody>{rows.map((row, index) => mode === "calls" ? <tr key={row.user_id || row.user_name || index}><td><strong>{value(row, "user_name")}</strong><small>{value(row, "user_id")}</small></td><td><span className="score">{number(row.score)}</span></td><td>{number(row.calls)}</td><td>{number(row.unique_leads)}</td><td>{number(row.handled_calls)}</td><td>{duration(row.average_duration_seconds)}</td><td>{value(row, "first_call")}</td><td>{value(row, "last_call")}</td></tr> : <tr key={row.author || index}><td><strong>{value(row, "author")}</strong></td><td>{number(row.notes)}</td><td>{number(row.unique_leads)}</td><td>{value(row, "first_note")}</td><td>{value(row, "last_note")}</td></tr>)}</tbody></table></div>}
  </article>;
}
...
export function NotesView({ data, page, setPage }) {
  const rows = data?.rows || [];
  return <><div className="section-heading"><div><span className="eyebrow">Notes</span><h2>Notes with the complete recording timeline</h2><p>Exact note/call matches appear first; every other recording for the same lead is attached below it.</p></div></div>
